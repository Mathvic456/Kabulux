...
export const WebSocketProvider = ({
  ...
  const [rideAccepted, setRideAccepted] = useState<RideAcceptedData | null>(
    null,
  );
  const [rideAcceptError, setRideAcceptError] =
    useState<RideAcceptErrorData | null>(null);
  ...
  const sendMessage = useCallback(
    async (payload: any) => {
      ...
      if (
        payload.type === "accept_ride" &&
        payload.data?.ride_request_view_id
      ) {
  ...
  const sendChatMessage = useCallback(
    async (rideId: string, text: string) => {
      const payload = {
        type: "send_message",
  ...
  const processMessageQueue = useCallback(() => {
    ...
    console.log(
      `📤 [WSP] Processing ${messageQueue.length} queued messages...`,
    );
  ...
  const connectWebSocket = useCallback(async () => {
    if (isReconnecting.current || manualDisconnect.current) {
      console.log(
        "⏸️ [WSP] Connection already in progress or manually disconnected",
      );
      return;
    }
    ...
    try {
      ...
      console.log(
        `🔌 [WSP] Connecting... (Attempt ${reconnectAttempts.current + 1}/${MAX_RECONNECT_ATTEMPTS})`,
      );
      ...
      newSocket.onclose = (event) => {
        console.log(
          `🔌 [WSP] Connection closed (Code: ${event.code}, Reason: ${event.reason || "Unknown"})`,
        );
